...
The data has 17 variables and covers 6759 shootings. Let's check out some of the variables, first using `head()` then using `summary()` and `table()`.
...
#> "2015-01-02" "2016-09-17" "2018-05-19" "2018-06-05" 
#>      3rd Qu.         Max. 
#> "2020-02-24" "2021-11-22"
summary(shootings$age)
#>    Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#>    6.00   27.00   35.00   37.12   45.00   92.00     334
```
...
#> 
#>     O     N     A     H     B     W 
#>  0.70  1.35  1.57 16.05 23.02 43.93
```
...
```r
ggplot(shootings, aes(x = age)) + geom_histogram()
...
```r
ggplot(shootings, aes(x = age)) + geom_histogram(bins = 15)
#> Warning: Removed 334 rows containing non-finite values
#> (stat_bin).
```
...
```r
ggplot(shootings, aes(x = age)) + geom_histogram(bins = 45)
#> Warning: Removed 334 rows containing non-finite values
#> (stat_bin).
```
...
```r
ggplot(shootings, aes(x = age)) + geom_histogram(aes(y = (..count..)/sum(..count..)))
#> `stat_bin()` using `bins = 30`. Pick better value
#> with `binwidth`.
#> Warning: Removed 334 rows containing non-finite values
#> (stat_bin).
...
```r
ggplot(shootings, aes(x = age)) + geom_density()
#> Warning: Removed 334 rows containing non-finite values
#> (stat_density).
```
...
```r
ggplot(shootings, aes(x = age)) + stat_count()
#> Warning: Removed 334 rows containing non-finite values
#> (stat_count).
```
...
```r
ggplot(shootings, aes(x = age)) + stat_count(aes(y = (..count..)/sum(..count..)))
#> Warning: Removed 334 rows containing non-finite values
#> (stat_count).
```
...
```r
ggplot(shootings, aes(x = race)) + geom_bar()
```
...
```r
shootings$race <- factor(shootings$race, levels = names(sort(table(shootings$race),
  decreasing = TRUE)))
```
...
```r
ggplot(shootings, aes(x = race)) + geom_bar() + coord_flip()
```
...
```r
ggplot(shootings, aes(x = race)) + geom_bar(aes(y = (..count..)/sum(..count..))) +
  coord_flip()
```
...
```r
shootings$dummy <- 1
ggplot(shootings, aes(x = date, y = dummy)) + geom_line()
```
...
```r
monthly_shootings <- aggregate(dummy ~ month_year, data = shootings,
  FUN = sum)
...
```r
yearly_shootings <- aggregate(dummy ~ year, data = shootings,
  FUN = sum)
ggplot(yearly_shootings, aes(x = year, y = dummy)) + geom_line()
```
