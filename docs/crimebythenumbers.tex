...
\NormalTok{shr\_2019\_and\_2020 }\OtherTok{\textless{}{-}}\NormalTok{ shr\_2019\_and\_2020[shr\_2019\_and\_2020}\SpecialCharTok{$}\NormalTok{homicide\_type }\SpecialCharTok{\%in\%} \StringTok{"murder and nonnegligent manslaughter"}\NormalTok{,]}
...
\CommentTok{\#\textgreater{} Warning: package \textquotesingle{}meditations\textquotesingle{} is in use and will not be installed}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\textgreater{} [1] "Do not have such an opinion of things as he has who does thee wrong, or such as he wishes thee to have, but look at them as they are in truth."}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{head}\NormalTok{(offenses\_known\_yearly\_1960\_2020}\SpecialCharTok{$}\NormalTok{agency\_name)}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{head}\NormalTok{(offenses\_known\_yearly\_1960\_2020}\SpecialCharTok{$}\NormalTok{actual\_murder)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{offenses\_known\_yearly\_1960\_2020}\SpecialCharTok{$}\NormalTok{agency\_name[}\DecValTok{15}\NormalTok{]}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{colorado }\OtherTok{\textless{}{-}}\NormalTok{ offenses\_known\_yearly\_1960\_2020[offenses\_known\_yearly\_1960\_2020}\SpecialCharTok{$}\NormalTok{state }\SpecialCharTok{==} \StringTok{"colorado"}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}
...
Instead, we'll first aggregate the data and then reshape that aggregated data. We'll aggregate the data by month and by day of week, and see how many people of each race were stopped at each month-day-of-week. In addition to \texttt{tidyr}, we'll use functions from \texttt{dplyr} to aggregate our data. Since we've already installed that package we just need to use \texttt{library()} and don't need to use \texttt{install.packages()} again. We've already used the \texttt{group\_by()} function in aggregating, and now we'll introduce a new function: \texttt{count()}. In our earlier aggregation code, we used the function \texttt{summarize()} and summed up columns which had a numeric variable (e.g.~the number of murders). \texttt{count()} works similarly by sums up categorical variables, which in our case is the race of people stopped. Using \texttt{count()} we enter in the categorical column in the parentheses and it'll make a new column called ``n'' which is the sum of each category. Since we're aggregating the data let's save the result of our aggregating to a new object called ``sqf\_agg''.
...
Now we want to reshape it again to go from wide to long. Our race columns names just took the values from the race column which means that we have column names all in capital letters and with spaces and slashes in it. We could technically use this as it is but it's a bit trickier to use any name with punctuation in it, and is against R column name convention, so we'll quickly fix this. To do so we'll use the package \texttt{make\_clean\_names()} function from the janitor package that automatically makes all character inputs to the function lower case and replaces all punctuation with an underscore. First we need to install the package with \texttt{install.packages()}.
...
To reshape this wide data to long format, we'll use the \texttt{tidyr} function \texttt{pivot\_longer()}. There are three inputs here: ``cols'' which takes a vector of column names which have our value variables, and ``names\_to'' which is what it'll call the newly created categorical variable, and ``values\_to'' which is what it'll call the newly created values column. For ``cols'' we want to include each of our race columns and these column names must be in quotes. For ``names\_to'' we can call it whatever we want but here we'll call it ``race'' as the variable is about the race of the person who was stopped. And for ``values\_to'' we'll call it ``number\_of\_people\_stopped'' though we can call it whatever we like.
...
\hypertarget{reshaping-a-multiple-columns}{%
\section{Reshaping a multiple columns}\label{reshaping-a-multiple-columns}}

So far we've just been reshaping using a single column. This is the simplest method, but in some cases we'll need to reshape using multiple columns. As an example, let's make a new column in our ``sqf\_agg'' dataset which just adds 10 to the value in our ``n'' column.
...
This now gives us two race columns - ``race'' and ``race2'' - which are ordered differently so we need to make sure to either reorder the data to be the same ordering or to keep that in mind when comparing the ``number\_of\_people\_stopped'' and ``number\_of\_people\_stopped2'' columns are they frequently refer to different races.
...
\CommentTok{\#\textgreater{}                                       }
\CommentTok{\#\textgreater{}                                       }
\CommentTok{\#\textgreater{}  ethanol\_beer\_gallons\_per\_capita ethanol\_wine\_gallons\_per\_capita}
\CommentTok{\#\textgreater{}  Min.   :1.210                   Min.   :0.1700                 }
\CommentTok{\#\textgreater{}  1st Qu.:1.310                   1st Qu.:0.1900                 }
\CommentTok{\#\textgreater{}  Median :1.350                   Median :0.2100                 }
\CommentTok{\#\textgreater{}  Mean   :1.344                   Mean   :0.2276                 }
\CommentTok{\#\textgreater{}  3rd Qu.:1.380                   3rd Qu.:0.2500                 }
\CommentTok{\#\textgreater{}  Max.   :1.450                   Max.   :0.3300                 }
...
\CommentTok{\#\textgreater{}  Min.   :33.74          Min.   : 93.43         Min.   :394.7         }
\CommentTok{\#\textgreater{}  1st Qu.:37.71          1st Qu.:105.89         1st Qu.:435.2         }
\CommentTok{\#\textgreater{}  Median :41.67          Median :126.65         Median :473.6         }
\CommentTok{\#\textgreater{}  Mean   :45.16          Mean   :123.31         Mean   :462.3         }
\CommentTok{\#\textgreater{}  3rd Qu.:49.61          3rd Qu.:141.18         3rd Qu.:497.1         }
\CommentTok{\#\textgreater{}  Max.   :65.49          Max.   :153.64         Max.   :509.9}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\textgreater{}      [,3]    [,4]                 [,5]        [,6]      [,7]        }
\CommentTok{\#\textgreater{} [1,] "Agent" "Other Than Mexican" "Marijuana" "Cocaine" "Accepted\textbackslash{}n"}
...
\CommentTok{\#\textgreater{} 1 Sector Profile {-} Fiscal Year 2017 (Oct. 1st through Sept. 30th)\textbackslash{}n\textbackslash{}n}
\CommentTok{\#\textgreater{}   apprehensions other\_than\_mexican\_apprehensions marijuana\_pounds}
\CommentTok{\#\textgreater{} 1         Agent               Other Than Mexican        Marijuana}
\CommentTok{\#\textgreater{}   cocaine\_pounds accepted\_prosecutions assaults     rescues}
\CommentTok{\#\textgreater{} 1        Cocaine            Accepted\textbackslash{}n   SECTOR Staffing*\textbackslash{}n}
...
\CommentTok{\#\textgreater{}   marijuana\_pounds cocaine\_pounds accepted\_prosecutions assaults}
\CommentTok{\#\textgreater{} 1        Marijuana        Cocaine            Accepted\textbackslash{}n   SECTOR}
\CommentTok{\#\textgreater{}       rescues}
\CommentTok{\#\textgreater{} 1 Staffing*\textbackslash{}n}
\CommentTok{\#\textgreater{}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               deaths}
\CommentTok{\#\textgreater{} 1 Apprehensions\textbackslash{}n                                                                                     Apprehensions                 (pounds)          (pounds)     Prosecutions\textbackslash{}n                                                                                                                                                               Assaults Rescues                 Deaths\textbackslash{}n\textbackslash{}nMiami                                             111              2,280                     1,646                  2,253              231               292              1              N/A      N/A\textbackslash{}nNew Orleans                                        63                920                      528                     21                 6               10               0              N/A      N/A\textbackslash{}nRamey                                              38               388                       387                     3               2,932              89               0           N/A         N/A\textbackslash{}nCoastal Border Sectors Total                      212              3,588                     2,561                  2,277             3,169              391              1         N/A ****    N/A ****\textbackslash{}n\textbackslash{}nBlaine                                            296                288                      237                      0                 0                9               0              N/A      N/A\textbackslash{}nBuffalo                                           277                447                      293                    228                 2               37               2              N/A      N/A\textbackslash{}nDetroit                                           408              1,070                      322                    124                 0               85               1              N/A      N/A\textbackslash{}nGrand Forks                                       189                496                      202                      0                 0               40               2              N/A      N/A\textbackslash{}nHavre                                             183                39                        28                     98                 0                2               0              N/A      N/A\textbackslash{}nHoulton                                           173                30                        30                     17                 0                2               0              N/A      N/A\textbackslash{}nSpokane                                           230                208                       67                     68                 0               24               0              N/A      N/A\textbackslash{}nSwanton                                           292               449                       359                    531                 1               103               6             N/A      N/A\textbackslash{}nNorthern Border Sectors Total                    2,048             3,027                     1,538                  1,066                3               302              11        N/A ****    N/A   ****\textbackslash{}nBig Bend (formerly Marfa)                         500              6,002                     3,346                  40,852              45              2,847             11             26       1\textbackslash{}nDel Rio                                          1,391             13,476                    6,156                  9,482               62              8,022             12             99       18\textbackslash{}nEl Centro                                         870              18,633                    5,812                  5,554              484              1,413             34             4        2\textbackslash{}nEl Paso                                          2,182             25,193                   15,337                  34,189             140              6,996             54             44       8\textbackslash{}nLaredo                                           1,666             25,460                    7,891                  69,535             757              6,119             31            1,054     83\textbackslash{}nRio Grande Valley (formerly McAllen)             3,130            137,562                  107,909                 260,020            1,192             7,979            422            1,190    104\textbackslash{}nSan Diego                                        2,199             26,086                    7,060                  10,985            2,903             3,099             84             48       4\textbackslash{}nTucson                                           3,691             38,657                   12,328                 397,090             331             20,963             93            750       72\textbackslash{}nYuma                                              859              12,847                   10,139                  30,181             261              2,367             33             6        2\textbackslash{}nSouthwest Border Sectors Total**                16,605            303,916                  175,978                 857,888            6,174            59,805            774            3,221    294\textbackslash{}nNationwide Total***                             19,437            310,531                  180,077                 861,231            9,346            60,498            786            3,221    294\textbackslash{}n* Agent staffing statistics depict FY17 on{-}board personnel data as of 9/30/2017\textbackslash{}n** Southwest Border Sectors staffing statistics include: Big Bend, Del Rio, El Centro, El Paso, Laredo, Rio Grande Valley, San Diego, Tucson, Yuma, and the Special Operations Group.\textbackslash{}n*** Nationwide staffing statistics include: All on{-}board Border Patrol agents in CBP\textbackslash{}n**** Rescue and Death statistics are not tracked for Northern and Coastal Border Sectors.}
...
\CommentTok{\#\textgreater{}    violatorswith\_a\_new\_charge pretrial\_misd conv\_misd bench\_warrants}
\CommentTok{\#\textgreater{} 23                          0             0         0              0}
\CommentTok{\#\textgreater{} 24                          0             0         0              0}
\CommentTok{\#\textgreater{} 25                         20             9         0              2}
\CommentTok{\#\textgreater{} 26                          0             3         0              0}
\CommentTok{\#\textgreater{} 27                          9             3         0              1}
\CommentTok{\#\textgreater{} 28                          2            26         1              2}
\CommentTok{\#\textgreater{}    federal pretrial\_sjf sentencedto\_co\_jail\_time}
\CommentTok{\#\textgreater{} 23       0            0                        0}
\CommentTok{\#\textgreater{} 24     164            0                        0}
\CommentTok{\#\textgreater{} 25       0            0                        3}
\CommentTok{\#\textgreater{} 26       0            0                        0}
\CommentTok{\#\textgreater{} 27       0           10                        1}
\CommentTok{\#\textgreater{} 28      19           13                        0}
\CommentTok{\#\textgreater{}    sjf\_sentencedto\_state\_jail total\_others total\_local total\_contract}
...
\CommentTok{\#\textgreater{}   address     lat  long place\_id licence   osm\_type osm\_id boundingbox}
\CommentTok{\#\textgreater{}   \textless{}chr\textgreater{}     \textless{}dbl\textgreater{} \textless{}dbl\textgreater{}    \textless{}int\textgreater{} \textless{}chr\textgreater{}     \textless{}chr\textgreater{}     \textless{}int\textgreater{} \textless{}list\textgreater{}     }
\CommentTok{\#\textgreater{} 1 750 Race\textasciitilde{}  40.0 {-}75.2   2.88e8 Data © O\textasciitilde{} way      6.22e7 \textless{}chr [4]\textgreater{}  }
\CommentTok{\#\textgreater{} \# ... with 4 more variables: display\_name \textless{}chr\textgreater{}, class \textless{}chr\textgreater{},}
\CommentTok{\#\textgreater{} \#   type \textless{}chr\textgreater{}, importance \textless{}dbl\textgreater{}}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\textgreater{}   address     lat  long matchedAddress tigerLine.tiger\textasciitilde{} tigerLine.side}
...
\CommentTok{\#\textgreater{} 1 750 Race\textasciitilde{}  40.0 {-}75.2 750 RACE ST, \textasciitilde{} 131423677        L             }
\CommentTok{\#\textgreater{} \# ... with 12 more variables: addressComponents.fromAddress \textless{}chr\textgreater{},}
...
\CommentTok{\#\textgreater{} \#   addressComponents.suffixType \textless{}chr\textgreater{}, ...}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\textgreater{}   address      lat  long arcgis\_address    score location.x location.y}
\CommentTok{\#\textgreater{}   \textless{}chr\textgreater{}      \textless{}dbl\textgreater{} \textless{}dbl\textgreater{} \textless{}chr\textgreater{}             \textless{}int\textgreater{}      \textless{}dbl\textgreater{}      \textless{}dbl\textgreater{}}
\CommentTok{\#\textgreater{} 1 750 Race \textasciitilde{}  40.0 {-}75.2 750 Race St, Phi\textasciitilde{}   100      {-}75.2       40.0}
\CommentTok{\#\textgreater{} \# ... with 4 more variables: extent.xmin \textless{}dbl\textgreater{}, extent.ymin \textless{}dbl\textgreater{},}
