...
The data has `r ncol(shootings)` variables and covers over 5,400 shootings. Let's check out some of the variables, first using `head()` then using `summary()` and `table()`.
...
From this we can see that the data is from January 1st through about a week ago. From the age column we can see that the average age is about `r round(mean(shootings$age))` with most people around that range. However, the youngest person is `r min(shootings$age)` years old while the oldest is `r max(shootings$age)`. Additionally, 182 rows have missing values for this variable. 
